...
class Test_Card(unittest.TestCase):
    def setUp(self):
        self.a_Card = Card(1, 3)
        self.b_Card = Card(1, 12)

    def test_string_method1(self):
        self.assertEqual(self.a_Card.__str__(), "3 of Clubs",
                         "testing string method Card with number value")

    def test_suit(self):
        self.assertEqual(self.a_Card.suit, "Clubs", "testing suit variable")

    def test_rank_num(self):
        self.assertEqual(self.a_Card.rank_num, 3, "testing rank_num variable")

    def test_rank(self):
        self.assertEqual(self.b_Card.rank, "Queen", "testing rank variable")

    def test_rank(self):
        self.assertEqual(self.b_Card.rank_num, 12, "testing rank_num variable")

    # Error #1: Doesn't give face value of cards
    def test_string_method2(self):
        c_Card = Card(2, 1)
        self.assertEqual(c_Card.__str__(), "Ace of Hearts", "testing string "
...
class Test_Deck(unittest.TestCase):
    ...
    def test_Deck1(self):
        self.assertEqual(type(self.a_Deck.cards), type([1, 2, 3]),
                         "testing cards variable is a list")

    def test_Deck2(self):
        Deck_string = self.a_Deck.__str__()
    ...
    def test_Deck4(self):
        self.a_Deck.deal_hand(2)
        self.assertEqual(len(self.a_Deck.cards), 50,
                         "testing deal_hand removes cards from deck")

    def test_Deck4(self):
        self.a_Deck.pop_card(2)
        self.assertEqual(len(self.a_Deck.cards), 51,
                         "testing that pop_card removes cards from deck")

    # Error #2: sort_cards does not sort remaining cards in deck: gives full
    #  deck length
    def test_Deck5(self):
        self.a_Deck.deal_hand(3)
        self.a_Deck.sort_cards()
        self.assertEqual(len(self.a_Deck.cards), 49, "testing that sort_cards "
    ...
    def test_Deck7(self):
        self.assertEqual(len(self.a_Deck.deal_hand(5)), 5,
                         "testing length of deal_hand")

    def test_Deck8(self):
        self.assertEqual(len(self.a_Deck.deal_hand(52)), 52, "testing maximum "
                         "length that deal_hand can take from deck")

    # Error #3: deal_hand cannot handle taking out large values from deck
    def test_Deck9(self):
        self.assertEqual(len(self.a_Deck.deal_hand(27)), 27, "testing large "
                         "inputs for deal_hand to take from deck")

    def test_Deck10(self):
        self.assertEqual(len(self.a_Deck.deal_hand(20)), 20, "testing medium "
                         "inputs for deal_hand to take from deck")


class Test_play_war_game(unittest.TestCase):
    def test_return(self):
        self.assertEqual(type(play_war_game(testing=True)), type((1, 2)),
                         "testing that return is a tuple")
